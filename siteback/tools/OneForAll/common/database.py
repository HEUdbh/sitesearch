...
class Database(object):
    ...
    def get_conn(self):
        """
        Get database connection with thread isolation

        :return: db_conn: SQLite database connection
        """
        # 检查当前线程是否已有连接
        if not hasattr(self._thread_local, 'conn'):
            logger.log('TRACE', f'Establishing database connection for thread {threading.get_ident()}')
            protocol = 'sqlite:///'
            if not self.db_path:  # 数据库路径为空连接默认数据库
                db_path = f'{protocol}{settings.result_save_dir}/result.sqlite3'
            else:
                db_path = f'{protocol}{self.db_path}'
            
            # 创建新的数据库连接，设置线程安全选项
            db = records.Database(db_path, 
                                 connect_args={'check_same_thread': False},
                                 poolclass=None)  # 禁用连接池，每个线程独立连接
            self._thread_local.conn = db.get_connection()
            logger.log('TRACE', f'Use the database: {db_path} in thread {threading.get_ident()}')
        
        return self._thread_local.conn

    @property
    def conn(self):
        """Property to get thread-local connection"""
